...
import type { Participacion, Prediccion } from '../../types/ParticipacionType';
...
const QuinielaDetail: React.FC = () => {
    ...
    const [userParticipation, setUserParticipation] = useState<Participacion | null>(null);

    const checkParticipationStatus = useCallback(async (quinielaId: number, usuarioId: number) => {
        try {
    ...
    }, [canUserParticipate, participacionesUsuario]);

    const loadUserPredictions = useCallback(async () => {
        if (userParticipation?.id) {
            await loadPrediccionesUsuario(userParticipation.id);
        } else {
            // Si no hay participación, limpiamos las predicciones para no mostrar datos antiguos
            clearPrediccionesActuales();
        }
    }, [userParticipation, loadPrediccionesUsuario, clearPrediccionesActuales]);
    ...
    // Este efecto carga las predicciones cuando cambia la participación del usuario
    useEffect(() => {
        if (quinielaActual?.id && userParticipation?.id) {
            loadUserPredictions();
        }
    }, [userParticipation, quinielaActual, loadUserPredictions]);
    ...
    const handleParticipate = async () => {
        ...
        try {
            ...
            if (result) {
                ...
                // Cargar predicciones para la nueva participación
                if (result.id) {
                    loadPrediccionesUsuario(result.id);
                }
            }
        } catch (error) {
    ...
    return (
        <div className="max-w-7xl mx-auto p-6">
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                                <div className="text-center">
                                    <div className="text-2xl font-bold text-green-600">
                                        ${(quinielaActual.poolActual|0).toLocaleString()}
                                    </div>
                                    <div className="text-sm text-gray-500">Pool Actual</div>
                                </div>
                                <div className="text-center">
                                    <div className="text-2xl font-bold text-blue-600">
                                        ${(quinielaActual.costoParticipacion|0).toLocaleString()}
                                    </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <dd className="text-lg text-gray-900 dark:text-white">${(quinielaActual.premioMinimo|0.).toLocaleString()}</dd>
                            </div>
                            <div>
            ...
            {user && (yaParticipaEnQuinielaActual || userParticipation) && (
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-xl font-semibold text-gray-900 dark:text-white">
                            Mis Predicciones
                        </h2>
                        {isLoadingPredicciones && <LoadingSpinner />}
                    </div>
                    ...
                    {isLoadingPredicciones ? (
                        ...
                        <div className="space-y-4">
                            {predicciones.map((prediccion: Prediccion, index: number) => (
                                <div key={index} className="border border-gray-200 dark:border-gray-600 rounded-lg p-4">
                                    <div className="flex justify-between items-start mb-2">
                                        <div>
                                            <h3 className="font-medium text-gray-900 dark:text-white">
                                                {prediccion.nombreEvento || `Evento ${index + 1}`}
                                            </h3>
                                            <p className="text-sm text-gray-500">
                                                {prediccion.descripcionEvento || 'Sin descripción'}
                                            </p>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mt-3">
                                        <div>
                                            <span className="text-sm text-gray-500">Mi Predicción:</span>
                                            <p className="font-medium text-gray-900 dark:text-white">
                                                {prediccion.prediccion}
                                            </p>
                                        </div>
                                        {prediccion.resultadoReal && (
                                            <div>
                                    ...
                                    {prediccion.puntosObtenidos !== undefined && (
                                        <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
                                            <span className="text-sm text-blue-600 font-medium">
                                                Puntos obtenidos: {prediccion.puntosObtenidos}
                                            </span>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 mt-4">
                                <div className="grid grid-cols-3 gap-4 text-center">
                                    <div>
                                        <div className="text-2xl font-bold text-green-600">
                                            {predicciones.filter(p => p.estado === 'ACIERTO').length}
                                        </div>
                                        <div className="text-sm text-gray-500">Aciertos</div>
                                    </div>
                                    <div>
                                        <div className="text-2xl font-bold text-red-600">
                                            {predicciones.filter(p => p.estado === 'FALLO').length}
                                        </div>
                                        <div className="text-sm text-gray-500">Fallos</div>
                                    </div>
                                    <div>
                                        <div className="text-2xl font-bold text-blue-600">
                                            {predicciones.reduce((sum, p) => sum + (p.puntosObtenidos || 0), 0)}
                                        </div>
                        ...
                        <div className="text-center py-8">
                            <div className="text-gray-500">
                                Aún no tienes predicciones registradas para esta quiniela.
                            </div>
                            {quinielaActual?.estado === EstadoQuiniela.ACTIVA && (
                                <button 
                                    className="mt-4 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg"
                                    onClick={() => window.location.href = `/usuario/quinielas/${quinielaActual.id}/predicciones`}
                                >
