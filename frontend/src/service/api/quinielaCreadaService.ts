 ...
 */
export const QuinielaCreadaService = {
...
crearQuiniela: async (request: CrearQuinielaRequest): Promise<QuinielaCreada> => {
    // Formateamos las fechas a formato ISO que el backend pueda manejar como LocalDateTime
  console.log("Request para crear quiniela:", request);
  ...
  getQuinielaById: async (id: number): Promise<QuinielaCreada> => {
    return QuinielaCreadaService.obtenerQuiniela(id);
  },
  ...
  buscarPorCodigo: async (codigo: string): Promise<QuinielaCreada> => {
    return QuinielaCreadaService.buscarPorCodigoInvitacion(codigo);
  }
};

// Singleton instance para compatibilidad con el código existente
let instance: typeof QuinielaCreadaService | null = null;

export const quinielaCreadaService = {
  getInstance: (): typeof QuinielaCreadaService => {
    if (!instance) {
      instance = QuinielaCreadaService;
    }
    return instance;
  }
};
...
// Export por defecto
export default QuinielaCreadaService;
